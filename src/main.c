/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hichikaw <hichikaw@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/28 00:00:00 by ichikawahik       #+#    #+#             */
/*   Updated: 2025/09/29 16:53:49 by hichikaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	print_usage(void)
{
	printf(
		"Usage: ./philo number_of_philosophers time_to_die time_to_eat "
		"time_to_sleep [must_eat]\n");
}

static int	launch_philos(t_state *state)
{
	int	idx;

	idx = 0;
	while (idx < state->config.number_of_philosophers)
	{
		if (pthread_create(&state->philos[idx].thread, NULL,
				philosopher_routine, &state->philos[idx]) != 0)
		{
			printf("Error: pthread_create failed\n");
			pthread_mutex_lock(&state->state_mutex);
			state->simulation_end = true;
			pthread_mutex_unlock(&state->state_mutex);
			return (1);
		}
		idx++;
	}
	return (0);
}

static void	join_threads(t_state *state)
{
	int	idx;

	idx = 0;
	while (idx < state->config.number_of_philosophers)
	{
		pthread_join(state->philos[idx].thread, NULL);
		idx++;
	}
	pthread_join(state->monitor_thread, NULL);
}

static int	start_monitor(t_state *state)
{
	if (pthread_create(&state->monitor_thread, NULL,
			monitor_routine, state) != 0)
	{
		printf("Error: pthread_create (monitor) failed\n");
		pthread_mutex_lock(&state->state_mutex);
		state->simulation_end = true;
		pthread_mutex_unlock(&state->state_mutex);
		return (1);
	}
	return (0);
}

int	main(int argc, char **argv)
{
	t_config	config;
	t_state		state;

	if (argc != 5 && argc != 6)
	{
		print_usage();
		return (1);
	}
	if (parse_args(argc, argv, &config) != 0)
		return (1);
	if (init_state(&state, &config) != 0)
		return (1);
	if (launch_philos(&state) != 0 || start_monitor(&state) != 0)
	{
		cleanup_state(&state);
		return (1);
	}
	join_threads(&state);
	cleanup_state(&state);
	return (0);
}
